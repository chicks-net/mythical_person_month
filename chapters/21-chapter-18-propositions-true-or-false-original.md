...
8.6 Aron's IBM data show productivity varying from 1.5 K lines of code (KLOC) per man-year to 10 KLOC/man-year as a function of the number of interactions among system parts.

8.7 Harr's Bell Labs data show productivities on operating-systems-type work to run about 0.6 KLOC/man-year and on compiler-type work about 2.2 KLOC/man-year for finished products.

8.8 Brooks's OS/360 data agrees with Harr's: 0.6-0.8 KLOC/man-year on operating systems and 2-3 KLOC/man-year on compilers.

8.9 Corbató's MIT Project MULTICS data show productivity of 1.2 KLOC/man-year on a mix of operating systems and compilers, but these are PL/I lines of code, whereas all the other data are assembler lines of code!
...
9.11 To make good space-time tradeoffs, a team needs to be trained in the programming techniques peculiar to a particular language or machine, especially a new one.
...
10.6 Preparing each document of this small set focuses thought and crystallizes discussion. The act of writing requires hundreds of mini-decisions, and it is the existence of these that distinguish clear, exact policies from fuzzy ones.
...
10.10 The project manager's chief daily task is communication, not decision-making; the documents communicate the plans and decisions to the whole team.
...
11.4 The discard and redesign may be done in one lump, or piece-by-piece, but it will be done.
...
12.11 Voluminosity in system documentation does indeed introduce a new kind of incomprehensibility [see Unix, for example], but it is far preferable to the severe underdocumentation that is so common.
...
13.8 Structured programming, designing programs whose control structures consist only of a specified set that govern blocks of code (versus miscellaneous branching), is a sound way to avoid bugs and is the right way to think.
...
14.4 Milestones must be concrete, specific, measurable events defined with knife-edge sharpness.

14.5 A programmer will rarely lie about milestone progress, if the milestone is so sharp he can't deceive himself.

14.6 Studies of estimating behavior by government contractors on large-scale development projects show that:
    - Estimates of the total time do not change as the project proceeds, but
    - Estimates of the time remaining to completion decrease only as the square root of the time already spent. (Pugh's Data)

14.7 Chronic schedule slippage is a morale-killer.

14.8 There is no substitute for a PERT chart or a critical-path-method chart for showing dependencies.

14.9 The preparation of a PERT chart is the most valuable part of its use, since one is forced to think through and document dependencies.

14.10 The first chart is always terrible, and one invents and invents in making the second one.

14.11 PERT charts with the activity on the arrows and the milestones at the nodes have been found to be much easier to use than the activity-on-node style.

14.12 Even on a small project, a PERT chart shows the essentials.

14.13 Laying out a network, identifying the dependencies, and estimating the legs of a network is a lot of work.

14.14 The estimate of the duration of each leg is very uncertain, depending on many variables.

14.15 A PERT chart has built-in statistical date estimators.

14.16 It is not clear one wants to use activity-time estimates that have confidence limits.

14.17 As the job proceeds, one can update the PERT estimates periodically for better information.

14.18 Hustle is essential; one must watch for small slippages and aggressively chase them down.

14.19 It is essential to have a quiet, controlled, and effective mechanism by which true status becomes known and secrets are smoked out.

14.20 Milestones are the answer.

14.21 A milestone has a single sharp completion event; an event that is either done or not.

14.22 If milestones are not sharp, one is reduced to checking PERT charts, which are very hard to do.

14.23 Chronic time-slippage is deadly for software development, for there is no one who can accurately predict how long a software project will take.

14.24 The secret to maintaining forward momentum is for the team to know where it is and to know that bosses know; know from sharp milestones and from reports that show true status.

14.25 Hustle, milestones, and the commitment to deliver good results are essential for maintaining forward momentum on projects.

## Chapter 15. The Other Face

15.1 Even software products that are well designed and functionally attractive can have serious acceptance problems if poorly documented.

15.2 The writing of the manual is the acid test for the consistency and completeness of the functional design.

15.3 Documentation falls into at least four categories:
    - To use a program
    - To believe a program
    - To modify a program
    - To verify or improve (i.e., understand in detail) a program

15.4 Many programs do not require all of these forms of documentation, or need them only to a reduced extent.

### To Use a Program

15.5 For a program to be easy to use, the description language for its input should be at a higher level than simple program input of numbers, characters, or bits.

15.6 For a program to be easy to use, it needs to furnish its user with feedback. All programs take longer than one expects to run.

15.7 If possible, a standard program should initialize itself to be usable in the expected case; "then user specification is required only to depart from the normal."

15.8 Having initialized to normal, a program should be ready to guide the user through the specification of options.

15.9 The best way to write any software user's manual is to describe in prose the first few examples used in designing the software; then add a reference guide in the form of a lookup manual.

### To Believe a Program

15.10 To believe a program, the first relevant concern is its range of applicability or its scope. What is it supposed to do? For what circumstances is it claimed valid?

15.11 One must also know what the accuracy or validity of the answers is.

15.12 Finally, one must know a self-checking case, with expected answers.

15.13 The test cases included in documentation must be complete, unambiguous, and stand-alone.

15.14 For some applications, fully worked test cases have to show intermediate values, not just results.

### To Modify a Program

15.15 To modify a program, the user needs to know its structure: a flow chart or subprogram hierarchy diagram helps him understand the pieces.

15.16 To modify a program, the user needs to understand each of the pieces.

15.17 To modify a program, the user needs to understand the algorithms that the pieces use.

15.18 Show the model; don't just show the algorithm.

15.19 Good documentation of a program intended for modification is not just desirable but vital.

### To Verify or Improve a Program

15.20 The first requisite for verifying or improving a program in detail is a rather clean, straightforward flow with no more subtleties than the inherent nature of the problem requires.

15.21 To verify or improve a program in detail, one especially needs the structure map (hierarchy chart).

15.22 To verify or improve a program in detail, one needs the data structure or data base layout.

15.23 When parts of the code are non-obvious, prose documentation embedded in the code as comments is invaluable.

15.24 Self-documenting code is a basic technique for giving the reader the most information per line scanned. The essence is choosing names for variables and functions so that they convey a maximum of useful information.

15.25 Mnemonic names mean something, but the reader has to remember what.

15.26 Labels, declarations, and the like should march down the page in column array, not be scattered horizontally, to ease scanning.

15.27 Indentation highlighting of the program structure is quite helpful.

15.28 Documentation must not only be complete and accurate, it must be currentr Obsolete documentation is actively dangerous.

### The Flow-Chart Curse

15.29 Much documentation is of poor quality, because it is hurriedly done.

15.30 Most documentation fails to give enough overview of the forest because of the understandable obsession of the documenter with the individual trees.

15.31 "The flow chart is a most thoroughly oversold piece of program documentation."

15.32 Flow charts show only the control structure, not the data structure; and the latter is often the more important.

15.33 Flow charts can be programmatically generated from the source code, and often are. In such cases, the flow chart is a redundant and not very useful derivative document.

15.34 Flow charts are for higher-level procedural documentation, if and where they are suitable at all.

## Chapter 16. No Silver Bullet

This chapter is included as Chapter 16 in this volume.

## Chapter 17. No Silver Bullet Refired

This chapter is included as Chapter 17 in this volume.

## Epilogue

### Vyssotsky's Question

In his review, Victor Vyssotsky challenges:

> I've found that teams **can** succeed in building complex systems, providing:
>
> a) Each piece of the system is simple—simple in the sense that one's confidence in it can be raised to an adequate level within a time and using resources one can afford to commit.
>
> b) The ways that the simple pieces fit together are simple—simple in the same sense.
>
> Can you find a criterion for simplicity (or confidence-building, or complexity) that can be applied to pieces and methods of interconnection, such that, when the criteria are met, we can believe the product will work?[^2]

This is a most important question. I cannot answer it, and I don't know who can, but I will tell what I think.

First, Vyssotsky's formulation is precisely correct—we must have a criterion for simplicity that applies to pieces and interconnections.

Second, to have such a criterion would be to have the key to the whole problem. In the real world, we start with a complex task and the question is whether we can chop it into a clean set of simple pieces and simple interconnections. Notice that the task itself is what it is; one cannot decree it to be simple.

Third, I think simplicity is inherently subjective, not objective, and it is inherently human-relative, not absolute. Hence I think the criterion for simplicity must be "understandable by a single human mind." (Not Vyssotsky's definition, which focused on confidence.) This means different things for different pieces; a piece devoted to a fancy algorithm for geometric computation must be simple to a numerical analyst; one doing database functions must be simple to a data structure expert.

Fourth, I believe in incremental growth. I would require the system to run usefully at each stage of growth—then the incremental complexities of successive growth can be more readily discerned.

Fifth, I believe that the only way to maintain confidence and control is a quite systematic approach to testing and checkout. One has to be able to show that (a) the pieces work, (b) the interfaces are correct, (c) the assemblages and subsystems work, (d) the whole works, (e) the performance meets the budget. I believe there should be at least as much effort in testing the components as is invested in building them, and at least as much effort in integration and system testing as in total component testing. All of these levels of testing must be systematic: test plans, recorded results, and cross-checks to ensure that tested results match designed behavior.

Finally, it seems to me the team-building problem and the people problem are critical. First, the system has to be designed by a very small number of minds—not many—so that it has conceptual integrity at the highest level. These minds have to be the best ones available.

Second, the implementation must also be done by the best available people. Here, however, is a paradox. The best people, in the sense of inherent mental qualities such as intelligence and judgment and creativeness, are not necessarily the best people in terms of hard-won expertise in particular areas. So the team must be carefully balanced—great minds who have worked hard to become experts where needed.

Finally, the team must cooperate. Great minds with great egos do not a team make. The surgical-team organization offers a way out of this dilemma.

### Murphy Still Lives

The fundamental truth of Murphy's Law is still with us: "If something can go wrong, it will." But another old truth is with us too: "God provides." He has generously provided the world not only with adequate economic resources and a heritage of useful knowledge, but also with fine minds and wonderful human spirits full of zest and goodwill. We can do very hard things if we work together and apply ourselves over time.

There is no royal road. But there is a road.

[^1]: B. W. Boehm, Software Engineering Economics, Englewood Cliffs: Prentice-Hall (1981), Chapter 21.

[^2]: V. A. Vyssotsky, private communication.
